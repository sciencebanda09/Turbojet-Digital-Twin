...
def generate_research_report(
    ...
    official_exps = [e for e in experiment_results if e.get("config", {}).get("split_strategy") == "official"]
    grouped_exps = [e for e in experiment_results if e.get("config", {}).get("split_strategy") == "grouped"]
    ...
    if official_exps and grouped_exps:
        ...
        for kind in sorted({e["config"]["kind"] for e in experiment_results}):
            ...
            if o:
                lines.append(f"| {kind} | official | {o['metrics'].get('rmse', 0):.4f} | {o['metrics'].get('r2', 0):.4f} | Same engines in train/test |")
            if g:
                lines.append(f"| {kind} | grouped  | {g['metrics'].get('rmse', 0):.4f} | {g['metrics'].get('r2', 0):.4f} | Held-out engines — harder |")
        lines.append("")
